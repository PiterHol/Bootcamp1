    ...
    <div>
        ...
        <div class="grid mx-6 gap-4 my-4">
            <div v-for="(wpis, index) in wpisy" class="grid drop-shadow-xl bg-stone-300 p-4">
                <p>id: {{ index }}</p>
                <p>{{ wpis }}</p>
                <button @click="usunWpisy(index)" class="bg-blue-600 rounded text-white p-4">usuń</button>
            </div>
        </div>
...
export default {
    ...
    methods: {
        ...
        async usunWpisy(index) {
            this.wpisy = await dzien2_backend.usun_wpisy(index);
            await this.pobierzWpisy();
        },
        async pobierzWpisy() {
